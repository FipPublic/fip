...
#define EASY_CURL_REQUEST_TIMEOUT 30000
...
{
public:
	RequestData(void) : RefObject(), m_body(NULL), m_callbackID(0){}
	RequestData(const char* method, const char* url, const char* body, uint32 body_length, uint32 nodeID, uint32 callbackID)
		: RefObject(), m_method(method), m_url(url), m_body(NULL), m_originNodeID(nodeID), m_callbackID(callbackID){
...
public:
	...
	Buffer* m_body;
	uint32 m_originNodeID;
	uint32 m_callbackID;
};

class EasyCurl : public EpollObject, public TimerObject, public RefObject, public Object0824
{
public:
