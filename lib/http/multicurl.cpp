...
int64 EasyCurl::timerCallback(void){
	//LOG_DEBUG("remove easy curl this=0x%x m_callbackID=%u", this, m_pRequest->m_callbackID);
...
//		fprintf(stderr, "set post data to http request\n");
	...
	//LOG_DEBUG("EasyCurl::initializeEasy this=0x%x m_callbackID=%u", this, m_pRequest->m_callbackID);
	return true;
}
...
//		}
        ...
        if (what == CURL_POLL_IN) {				// 1
            events |= EPOLLIN;
			//LOG_DEBUG("EasyCurl::socketCallback Read this=0x%x m_callbackID=%u", pEasyCurl, pEasyCurl->m_pRequest->m_callbackID);
        } else if (what == CURL_POLL_OUT) {		// 2
            events |= EPOLLOUT;
			//LOG_DEBUG("EasyCurl::socketCallback Write this=0x%x m_callbackID=%u", pEasyCurl, pEasyCurl->m_pRequest->m_callbackID);
